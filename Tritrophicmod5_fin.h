/*
Tritrophicmod5_fin.h - Header file specifying the elementary life-history functions of 
...
The model includes two basic resource, two size-structured consumers
and an unstructured predator population;
_ Rho1 for R1, Rho2 for R2. Rho2 = 2*Rho1
_ BSRatio only implicated in the state at birth and the maximum length
Assumptions : Bigger individuals grow slower and maturate later

Layout for the i-state variables of the structured population i:
istate[n][0] : Age (n)
istate[n][1] : Length (n)
...
E[1] : Resource R2
E[2] : Predators
E[3] : Vulnerable consumer biomass B1
E[4] : Vulnerable consumer biomass B2

Layout for the interaction (and output) variables of the structured population i:
I[i][0] : Total ingestion of the resource R1 by the species i
I[i][1] : Total ingestion of the resource R2 by the species i
I[i][2] : Total biomass of small juveniles
I[i][3] : Total biomass of non-vulnerable juveniles
I[i][4] : Total adult biomass
*/
...
{ "Lb", "Lj", "Lm", "Omega", "Imax", "Rh1", "Rh2", "Nu", "Rm", "Mub", "Rmax1", "Rmax2", "Rho1", "Rho2", "Lv", "A", "Th", "Epsilon", "Delta", "Alpha", "Bsratio" };

// Default values of all parameters 
double	parameter[PARAMETER_NR] =
{ 7.0, 110.0, 300.0, 9.0E-6, 1.0E-4, 1.5E-5, 1.5E-5, 0.006, 0.003, 0.01, 5E-6, 3E-4, 0.1, 0.2, 27.0, 5000.0, 0.1, 0.5, 0.01, 0.0, 1.2}; //
	...
	double mortality[POPULATION_NR])
{
	for(int i=0; i<2; i++)  {
		if (lifestage[i] == 0)
			mortality[i] = MUB + A*P/(1+A*TH*(B1 + B2));
		else
			mortality[i] = MUB;
	}
	return;
